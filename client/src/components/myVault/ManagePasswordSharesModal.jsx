    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white rounded-2xl p-6 shadow-xl">
        ...
        <div className="rounded-xl bg-slate-50 p-4 mb-4">
          ...
          <p className="font-semibold text-slate-900">{password?.name}</p>
        </div>

        {loading ? (
          <p className="text-sm text-slate-500">Loading shared users...</p>
        ) : shares.length === 0 ? (
          <p className="text-sm text-slate-500">No users have access.</p>
        ) : (
